...
static const char* mqtt_broker_uri = "mqtts://IOTAssign.azure-devices.net";
...
static void generateTelemetryPayload()
{
  ...
  telemetry_payload ="{"
                    ...
                    "\"cabin Temperature\": " + String(temp1) + ","
                    "\"CabinHumidity\": " + String(humidity1) + ","
                    "\"engine temp\": " + String(temp2) + ","
                    "\"number of shocks\": " + String(shockno) +
                    "}";
  /*********************************************************************************************/
}
...
String data = String(incomingPacket);
    ...
    temp1 = data.substring(0, data.indexOf(',')).toFloat();
    data = data.substring(data.indexOf(',') + 1); // Remove parsed part

    // Parse humidity1
    humidity1 = data.substring(0, data.indexOf(',')).toFloat();
    data = data.substring(data.indexOf(',') + 1); // Remove parsed part

    // Parse temp2
    temp2 = data.substring(0, data.indexOf(',')).toFloat();
    data = data.substring(data.indexOf(',') + 1); // Remove parsed part

    // Parse pirval1
    pirval1 = data.substring(0, data.indexOf(',')).toInt();
    data = data.substring(data.indexOf(',') + 1); // Remove parsed part

    // Parse shockno
    shockno = data.substring(0, data.indexOf(',')).toInt();
